...
def show_2019(request, course, education_category):
    ...
    locs = LearningOutcomeCharacteristic.get_for_course_level(
        course.level
    ).filter(
        education_category=education_category
    )

    context['remaining_locs'] = locs.exclude(locs__in=clos)
    template_path = os.path.join(TEMPLATE_ROOT, 'show.html')
    return render(request, template_path, context)


def add_2019(request, course, education_category):
    queryset = LearningOutcomeCharacteristic.get_for_course_level(
        course.level
    ).filter(
        education_category=education_category
    )
    ...
    form = CourseLearningOutcomeLocsForm(
...
def update_2019(request, course, education_category, clo):
    queryset = LearningOutcomeCharacteristic.get_for_course_level(
        course.level
    ).filter(
        education_category=education_category
...
def print_pdf_2019(request, course):
