...
class SubjectToModuleLearningOutcomeForm(forms.ModelForm):
    class Meta:
        model = SubjectToModuleLearningOutcome
        exclude = []

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['loes'].required = False

    def clean_loes(self):
        data = self.cleaned_data['loes']
        if len(data) > 3:
            raise forms.ValidationError("Możesz wybrać do 3 pozycji.")
        return data


SubjectToModuleLearningOutcomeFormset = inlineformset_factory(
    Subject,
    SubjectToModuleLearningOutcome,
    form=SubjectToModuleLearningOutcomeForm,
    exclude=(),
    widgets={
        'mlo': forms.HiddenInput,
        'loes': forms.CheckboxSelectMultiple
    },
    extra=0,
    can_delete=False
...
class SyllabusToEctsForm(forms.ModelForm):
    class Meta:
        model = SyllabusToECTS
        exclude = []

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['ects'].widget = forms.HiddenInput()
