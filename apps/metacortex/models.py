...
class SyllabusToECTS(models.Model):
    ...
    syllabus = models.ForeignKey('Syllabus', related_name="syllabus_to_ects")
    ects = models.ForeignKey('ECTS')
...
def subject_to_teacher_post_save(instance, created, **kwargs):
    if created:
        ...
        if subject.type.is_practice():
            SyllabusPractice.objects.update_or_create(subject=subject, teacher=teacher, defaults={'is_active': True})
        else:
